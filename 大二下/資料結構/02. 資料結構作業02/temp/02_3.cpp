...
bool chack_point_passed(vector<vector<vector<int>>> map, vector<road_init> &road, int now[3]) // 檢查點是否走過
{
...
void back_road(vector<road_init> &road) // 回退
{
    // cout<<"回退"<<endl;
...
int main()
{
    ...
    while (true)
    {
        ...
        // 起點標記為走過
        visited[start_x-1][start_y-1][start_z-1] = true;
        // print_map(map, x, y, z);
        ...
        if (chack_point_dir(map, x, y, z, road[road.size() - 1]))
        {
            while (true)
            {
                ...
                else if (road[road.size() - 1].direction[0] == 1) // 可以向右
                {
                    ...
                    now[2]++;
                    if (chack_point_passed(map, road, now)) // 檢查是否走過
                    {
